...
def predict_next_month():
    ...
    Uses latest uploaded file path.
    """
    ...
    for product in df["Product"].unique():
        ...
        recommended_qty = int(round(pred_qty * 1.1))

        investment = round(recommended_qty * float(last_row["Cost_Price"]), 2)
        profit = (float(last_row["Selling_Price"]) - float(last_row["Cost_Price"])) * pred_qty
        margin = round(
            ((float(last_row["Selling_Price"]) - float(last_row["Cost_Price"])) /
